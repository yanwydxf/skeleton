		...
		<view class="skeleton" v-show="loading" :class="[animationClass]" v-for="skeleton in count">
			<view class="row" >
				<view v-for="(item,index) in row" :key="item">
					<view :style="{width:rowWidth,height:rowHeight}" v-bind="$attrs" v-if="variant === 'image'" :class="[varianttype]">
					...
					<view v-else :class="[varianttype]" :style="{width:rowWidth,height:rowHeight}" v-bind="$attrs" class="row-class"></view>
				</view>
			</view>
		</view>
